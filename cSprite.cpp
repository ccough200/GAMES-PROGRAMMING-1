...
// Default Constructor
cSprite::cSprite() {
	setSpriteScaling(1.0f, 1.0f);
	setSpriteScalingRotation(0.0f);
	setSpriteRotation(0.0f);
}

// Constructor
cSprite::cSprite(D3DXVECTOR3 sPosition, LPDIRECT3DDEVICE9 pd3dDevice, LPCSTR theFilename) {
	cSprite::setSpritePos(sPosition);
	cSprite::setTexture(new cD3DXTexture(pd3dDevice, theFilename));
...
// Constructor
cSprite::cSprite(D3DXVECTOR3 sPosition, D3DXVECTOR2 translation, LPDIRECT3DDEVICE9 pd3dDevice, LPCSTR theFilename) {
	setSpritePos(sPosition);
	setTexture(new cD3DXTexture(pd3dDevice, theFilename));
...
/*
=================
- update the sprite position based on the translation
=================
*/
void cSprite::update(float deltaTime)
{
	D3DXVECTOR2 tempV2;
	D3DXVECTOR3 tempV3;
	tempV2 = cSprite::getSpritePos2D() + getTranslation()*(deltaTime/1000.0f);
...
// Return true if a point is within the rectangle
bool cSprite::insideRect(RECT thisSprite, POINT aPoint) {
	if ((aPoint.x >= thisSprite.left) && (aPoint.x <= thisSprite.right) && (aPoint.y >= thisSprite.top ) && (aPoint.y <= thisSprite.bottom))
		return true;
	else 
		return false;
}